...
router.get("/status", auth, async (req,res) => {
  try {
    ...
    res.json({success:true, bonus:{
      progress:Number(user.bonusProgress||0), target:Number(user.bonusTarget||0),
      quizProgress:Number(user.bonusQuizProgress||0), learningProgress:Number(user.bonusLearningProgress||0),
      unlocked:!!user.bonusUnlocked, claimed:!!user.bonusClaimed, source:user.bonusSource||"",
...
router.post("/claim", auth, async (req,res) => {
  try {
    ...
    await user.save();
    res.json({success:true,reward,wallet:Number(user.wallet||0),totalEarn:Number(user.totalEarn||0),source:user.bonusSource||""});
  } catch(e){ res.status(500).json({success:false,message:e.message}); }
});
